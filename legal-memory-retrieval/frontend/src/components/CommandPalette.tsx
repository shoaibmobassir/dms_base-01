...
export function CommandPalette({ onClose }: { onClose: () => void }) {
  const { matters, documents, clients, projects } = useApp()
  ...
  const results = useMemo(() => {
    ...
    const items: Array<{ kind: string; label: string; path: string }> = [
      ...
      { kind: 'Nav', label: 'Ask Firm', path: '/ask' },
    ...
    return items
      ...
      .slice(0, 16)
  }, [q, matters, documents, clients, projects])
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-head">
          <input
            ...
            style={{ flex: 1 }}
            placeholder="Jump to a matter, document, or view…"
            value={q}
